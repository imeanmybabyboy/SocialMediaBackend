...
namespace SocialMediaBackend.Data
{
    public static class SeedData
    {
        ...
        public static PostLike[] PostsLikes(User[] users, Post[] posts)
        {
            var random = new Random();
            var postLikesList = new List<PostLike>();

            foreach(var user in users)
            {
                int likesCount = random.Next(0, posts.Length + 1);
                if (likesCount == 0) continue;
                var shuffledPosts = posts.OrderBy(_ => random.Next()).Take(likesCount);

                foreach (var post in shuffledPosts)
                {
                    postLikesList.Add(new PostLike
                    {
                        UserId = user.Id,
                        PostId = post.Id,
